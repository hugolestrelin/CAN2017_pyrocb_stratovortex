# -*- coding: utf-8 -*-
"""
Shows the sequence of CALIOP sectiosn in the vicinity of the August 2017
...
with gzip.open('/scratch/hlestrelin/fig_ALA2014/selCaliop_ALA2014.pkl','rb') as f:
...
#    ysup = 38
    ...
    GEO = False
    #gg = geosat.GeoGrid('HimFull')
    #subgg = gg.subgrid([230,350,40,75])
    cm = 0
...
#%% Here we get the himawari scene which is the closest for this time
    ...
    if figsav:
        plt.savefig('/scratch/hlestrelin/fig_ALA2014/plot_ALA2014'+'.'+str(i)+'.png',dpi=300,bbox_inches='tight')
    if show: plt.show()
    plt.close(fig=fig)
    #if AProOK:
    #    hh.close()
