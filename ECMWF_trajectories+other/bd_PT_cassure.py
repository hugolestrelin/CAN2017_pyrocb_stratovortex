...
## 31/08
itrack=0
ax0 = fig.add_subplot(gs[3,1],projection=proj)
date00=datetime(2017, 8, 31, 18, 0)
track_model3['dates'][itrack]=datetime(2017, 8, 31, 3, 0)
...
if track_model3['dates'][itrack].month==8:
    textstr='i)\n'+str(track_model3['dates'][itrack].day)+' Aug\nPT = '+str(int(PTref))+' K'#\n('+str(PTref2)+')'
elif track_model3['dates'][itrack].month==9:
    textstr='i)\n'+str(track_model3['dates'][itrack].day)+' Sept\nPT = '+str(int(PTref))+' K'#\n('+str(PTref2)+')'
...
#ax1 = plt.subplot(521,sharex=ax0 ,sharey=ax0,projection = proj)
...
## 23/08
itrack=2
ax2 = plt.subplot(523,sharex=ax0, sharey=ax0,projection = proj)
date00=track_model3['dates'][itrack]
namedate00=savfile+'dats_'+str(date00.day)+'_'+str(date00.month)+'_i_PT='+str(itrack)
if os.path.isfile(namedate00):
    with gzip.open(namedate00,'rb') as f:
    ...
    if CALIOPtrack[j]['date'].day==date00.day:
        if export[j]['lats'][0]>export[j]['lats'][1]:
            if j==18:
                ...
                ax7.plot(export[j]['lons'][np.where(export[j]['lats']<=latrangemaxlats)[0][0]:np.where(export[j]['lats']<=latrangemin)[0][0]],export[j]['lats'][np.where(export[j]['lats']<=latrangemaxlats)[0][0]:np.where(export[j]['lats']<=latrangemin)[0][0]],'limegreen',linewidth=lnwd,label='detailed orbits')#,linewidth=1,linestyle='-')
                ax7.plot(export[j]['lons'][np.where(export[j]['lats']<=CALIOPtrack[j]['north'])[0][0]:np.where(export[j]['lats']<=CALIOPtrack[j]['south'])[0][0]],export[j]['lats'][np.where(export[j]['lats']<=CALIOPtrack[j]['north'])[0][0]:np.where(export[j]['lats']<=CALIOPtrack[j]['south'])[0][0]],'red',linewidth=lnwd)#,linewidth=1,linestyle='-')
