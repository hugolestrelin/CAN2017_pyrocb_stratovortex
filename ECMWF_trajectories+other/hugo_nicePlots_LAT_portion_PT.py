...
initials='AUS2009'
nametrackmodel3 = '/home/hlestrel/stageM1-vortex/Vortex-track_'+initials+'.pkl'#+'_B2.pkl'
savfile='/data/hlestrel/fig_bulle_'+initials+'/fig_publi/PT_portion/'
variable='PV'
...
altitude=track_model3['PT'][idebut]
...
if os.path.isfile(savfile+'dats00PT_'+str(idebut)):
    with gzip.open(savfile+'dats00PT_'+str(idebut),'rb') as f:
    ...
    with gzip.open(savfile+'dats00PT_'+str(idebut),'wb') as f:
        pickle.dump(dats00,f)

        
## map de données :
jy = np.where(dats00.attr['lats']>=lat_bulle)[0][0]
ialts=np.where(dats00.var['PTmean'][:,jy]<=altitude)[0][0] 
#events=[170,195,218,233,254] #CAN2017
#events=[120,140,165] #130 , 150
events=[120,140,165]
...
for itrack in events:
    ...
    namedate00=savfile+'dats00PT_'+str(date00.day)+'_'+str(date00.month)+'_i='+str(itrack)
    ...
    pos= np.where(dats00.attr['lats']>=track_model3['lats'][itrack])[0][0]
    alt_min_w=np.where(dats00.var['PTmean'][:,pos]<=track_model3['PT'][itrack]-fenetre_alt*0.1)[0][0]
    alt_max_w=np.where(dats00.var['PTmean'][:,pos]<=track_model3['PT'][itrack]+fenetre_alt*0.9)[0][0]
    ...
    kz1 = np.where(dats00.var['PTmean'][:,pos]<=track_model3['PT'][itrack])[0][0]
...
#### PLOT ZONAL
lat=lat_bulle
ialtrangemin=np.where(dats.var['PT']<=altrangemin)[0][0]
ialtrangemax=np.where(dats.var['PT']<=altrangemax)[0][0]
...
try:
    ...
    iax2=ax2.pcolormesh(lons,dats00.var['PTmean'][l1:l2+2,pos,:],scale*dats00.var[variable+'anoZ'][l1:l2+1,pos, :],
                vmin=clim[0],vmax=clim[1],cmap=cmap)
...
for itrack in events:
    ...
    alt_min_w=track_model3['PT'][itrack]-fenetre_alt*0.37
    alt_max_w=track_model3['PT'][itrack]+fenetre_alt*0.7-6
    ...
    ax2.plot([lon_min_w,lon_min_w],[alt_min_w,alt_max_w],linewidth=2.5,color='black') # gauche
    ax2.plot([lon_max_w,lon_max_w],[alt_min_w,alt_max_w],linewidth=2.5,color='black') # droite
    ax2.plot([lon_min_w,lon_max_w],[alt_max_w,alt_max_w],linewidth=2.5,color='black') # haut
    ax2.plot([lon_min_w,lon_max_w],[alt_min_w,alt_min_w],linewidth=2.5,color='black') # bas
    xleg2,yleg2=proj2legend(track_model3['lons'][itrack]-9.2,track_model3['PT'][itrack]-altrangemin-35,lonrangemin,lonrangemax,altrangemin,altrangemax)
    ...
    elif track_model3['dates'][itrack].month==2:
        textstr2=str(track_model3['dates'][itrack].day)+' Feb'
    elif track_model3['dates'][itrack].month==3:
        textstr2=str(track_model3['dates'][itrack].day)+' Mar'
    ax2.set_ylim(altrangemin,altrangemax)
    ax2.text(xleg2,yleg2,textstr2, transform=ax2.transAxes, fontsize=12, verticalalignment='top', bbox=props2)

for i in range(idebut,165,itera): #263 CAN2017
    if  track_model3['dates'][i].month==9 and 3<track_model3['dates'][i].day<16:
        ax2.plot([lons3[i-itera],lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color='k',marker='x',alpha=0.5)
    else:
        ax2.plot([lons3[i-itera],lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color='k',alpha=0.5)

plt.xticks([30,60,90,120,150,180],['30°E','60°E','90°E','120°E','150°E','180°E'])
plt.savefig(savfile+str(variable)+'_lat_PT.png',bbox_inches='tight',dpi=300,format='png')
plt.show()