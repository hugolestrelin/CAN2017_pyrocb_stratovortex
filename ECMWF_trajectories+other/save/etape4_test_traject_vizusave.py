...
if visualization_track:
    nametrackmodel='/home/hlestrel/stageM1-vortex/track_model_'+initials+'debutest'
    inputest=False
    import matplotlib.colors as colors
    import cartopy.crs as ccrs
    from cartopy import feature
    ...
    if os.path.isfile(nametrackmodel) :
        with gzip.open(nametrackmodel,'rb') as f:
            track_model = pickle.load(f)
        #print(track_model)
        #while date00.day!=12 or date00.hour!=0:
        for i in range(189,281,3):
            idx=i
            date00=track_model[idx]['date']
            ...
            ax=datsvisu.show('PV',ialts,projec=None,show=False,txt='Potential vorticity at altitude '+str(round(altitude,2)),clim=(-0.1*10**-5,1.5*10**-5))#,savfile=dirout+'PValt-'+str(track_model[idx]['date'])+str(idx)+'.png'
            ...
            savfile1=dirout+'PValt-'+str(date00)+str(idx)+'.png'
            plt.savefig(savfile1,bbox_inches='tight',dpi=300,format='png')
            
            ax2=datsvisu.chartlonz('PV',lat=lat_bulle,levs=(altmax,altmin),show=False,txt='Potential vorticity',clim=(-0.1*10**-5,1.5*10**-5))#,savfile=dirout+'PVlat-'+str(track_model[idx]['date'])+str(idx)+'.png'
            ...
            #plt.connect('key_press_event', toggle_selector)
            #plt.show()
            savfile2=dirout+'PVlat-'+str(date00)+str(idx)+'.png'
...
if print_traj:
    nametrackmodel1 = '/home/hlestrel/stageM1-vortex/track_model_'+initials+'_BulleA_ref_tot'
    nametrackmodel2 = '/home/hlestrel/stageM1-vortex/track_model_'+initials+'_BulleB1_ref_tot'
    nametrackmodel3 = '/home/hlestrel/stageM1-vortex/track_model_'+initials+'_BulleB2_ref_tot'
    savfile='/home/hlestrel/stageM1-vortex/'
    variable='O3'
    ...
    with gzip.open(nametrackmodel2,'rb') as f:
        track_model2 = pickle.load(f)
    with gzip.open(nametrackmodel3,'rb') as f:
        track_model3 = pickle.load(f)
    idx1=len(track_model1)-1
    idx2=len(track_model2)-1
    idx3=len(track_model3)-1
    date00=track_model2[200]['date'] #choix de l'indice au niveau de la séparation en trois du panache
    altitude=track_model2[200]['alt']
    lon_mid=track_model2[200]['lon'] 
    lat_bulle=track_model2[200]['lat']
    if (lon_mid+fenetre_lon/2)<359:
        loninf = lon_mid-fenetre_lon/2
        lonsup = lon_mid+fenetre_lon/2
    elif (lon_mid+fenetre_lon/2)>359:
    ...
    dat._mkpscale() 
    dat._mkzscale()
    ialts=track_model1[200]['ialts']
    ilons=track_model1[200]['ilons']
    ilats=track_model1[200]['ilats']
    min_O3=track_model1[200]['min_O3']
    O3mean = np.mean(dat.var[variable],axis=2)
    dat.var[variable+'ano'] = dat.var[variable] - O3mean[:,:,np.newaxis]
    #if (loninf < 0):
    datp=dat.shift2west(180) 
    ...
    #     dats = dat.extract(varss='All',lonRange=(loninf,lonsup),latRange=(latinf,latsup)) 
    #     if lat_bulle>0:
    #         dats00 = dat.extract(varss='All',lonRange=(-180,180),latRange=(10,85))
    #     elif lat_bulle<0:
    #         dats00 = dat.extract(varss='All',lonRange=(-180,180),latRange=(-80,-3))
    #datsvisu.show(variable,ialts,cLines=False,projec=None,show=False)
    
    #datsvisu.chartlonz(variable,lat=dats.attr['lats'][ilats],levs=(ialts-6,ialts+6),show=False)
    jet1= plt.get_cmap('binary') # jet
    colors1 = iter(jet1(np.linspace(0,1,colori)))
    colors12 = iter(jet1(np.linspace(0,1,colori))) # attention à bien adapter au nombre de points que l'on souhaite tracer
    jet2= plt.get_cmap('Blues') 
    colors2 = iter(jet2(np.linspace(0,1,colori))) 
    colors22 = iter(jet2(np.linspace(0,1,colori))) 
    jet3= plt.get_cmap('Reds')
    ...
    alts1=[]
    for i in range(idx1):
        alts1.append(track_model1[i]['alt'])
        lats1.append(track_model1[i]['lat'])
        if track_model1[i]['lon']>180:
            lons1.append(track_model1[i]['lon']-360)
        elif track_model1[i]['lon']<-180:
            lons1.append(track_model1[i]['lon']+360)
        else:
            lons1.append(track_model1[i]['lon'])
    lons2=[]
    lats2=[]
    alts2=[]
    for i in range(idx2):
        alts2.append(track_model2[i]['alt'])
        lats2.append(track_model2[i]['lat'])
        if track_model2[i]['lon']>180:
            lons2.append(track_model2[i]['lon']-360)
        elif track_model2[i]['lon']<-180:
            lons2.append(track_model2[i]['lon']+360)
        else:
            lons2.append(track_model2[i]['lon'])
    lons3=[]
    lats3=[]
    alts3=[]
    for i in range(idx3):
        alts3.append(track_model3[i]['alt'])
        lats3.append(track_model3[i]['lat'])
        if track_model3[i]['lon']>180:
            lons3.append(track_model3[i]['lon']-360)
        elif track_model3[i]['lon']<-180:
            lons3.append(track_model3[i]['lon']+360)
        else:
            lons3.append(track_model3[i]['lon'])
    itera=4
    lnwd=3
...
##    ax2=dats00.chartlonz(variable+'ano',lat=dats.attr['lats'][ilats],levs=(ialts-20,ialts+20),show=False,txt=str(variable)+' lat')
    ...
    txt='Trajectories of three followed vortex on an latitudinal ozone map (10**-6 g/kg)'
    log=False
    clim=(None,None)
    cmap=mymap
    scale=10**6
    ...
    for i in range(itera,idx1+1,itera):
        if  track_model1[i]['date'].month==9 and 3<track_model1[i]['date'].day<16:
            if (lons1[i-itera]-lons1[i]<-100): 
                ax2.plot([180,lons1[i]],[alts1[i-itera],alts1[i]],linewidth=lnwd,color='black',marker='x')
            elif (lons1[i-itera]>150 and lons1[i])<-150:
                ax2.plot([-180,lons1[i]],[alts1[i-itera],alts1[i]],linewidth=lnwd,color='black',marker='x')
            else:
                ax2.plot([lons1[i-itera],lons1[i]],[alts1[i-itera],alts1[i]],linewidth=lnwd,color='black',marker='x')
        elif i==int(idx1/itera)*itera:
            if (lons1[i-itera]-lons1[i]<-100): 
                ax2.plot([180,lons1[i]],[alts1[i-itera],alts1[i]],linewidth=lnwd,color=next(colors12),label='Bubble A')
            elif (lons1[i-itera]>150 and lons1[i])<-150:
                ax2.plot([-180,lons1[i]],[alts1[i-itera],alts1[i]],linewidth=lnwd,color=next(colors12),label='Bubble A')
            else:
                ax2.plot([lons1[i-itera],lons1[i]],[alts1[i-itera],alts1[i]],linewidth=lnwd,color=next(colors12),label='Bubble A')
        else:
            if (lons1[i-itera]-lons1[i]<-100): 
                ax2.plot([180,lons1[i]],[alts1[i-itera],alts1[i]],linewidth=lnwd,color=next(colors12))
            elif (lons1[i-itera]>150 and lons1[i])<-150:
                ax2.plot([-180,lons1[i]],[alts1[i-itera],alts1[i]],linewidth=lnwd,color=next(colors12))
            else:
                ax2.plot([lons1[i-itera],lons1[i]],[alts1[i-itera],alts1[i]],linewidth=lnwd,color=next(colors12))
    for i in range(itera,idx2+1,itera):
        if  track_model2[i]['date'].month==9 and 3<track_model2[i]['date'].day<16:
            if (lons2[i-itera]-lons2[i]<-100): 
                ax2.plot([180,lons2[i]],[alts2[i-itera],alts2[i]],linewidth=lnwd,color='blue',marker='x')
            elif (lons2[i-itera]>150 and lons2[i])<-150:
                ax2.plot([-180,lons2[i]],[alts2[i-itera],alts2[i]],linewidth=lnwd,color='blue',marker='x')
            else:
                ax2.plot([lons2[i-itera],lons2[i]],[alts2[i-itera],alts2[i]],linewidth=lnwd,color='blue',marker='x')
        elif i==int(idx2/itera)*itera:
            if (lons2[i-itera]-lons2[i-1]<-100): 
                ax2.plot([180,lons2[i-1]],[alts2[i-itera],alts2[i-1]],linewidth=lnwd,color=next(colors22),label='Bubble B1')
            elif (lons2[i-itera]>150 and lons2[i-1])<-150:
                ax2.plot([-180,lons2[i-1]],[alts2[i-itera],alts2[i-1]],linewidth=lnwd,color=next(colors22),label='Bubble B1')
            else:
                ax2.plot([lons2[i-itera],lons2[i-1]],[alts2[i-itera],alts2[i-1]],linewidth=lnwd,color=next(colors22),label='Bubble B1')
        else:
            if (lons2[i-itera]-lons2[i]<-100): 
                ax2.plot([180,lons2[i]],[alts2[i-itera],alts2[i]],linewidth=lnwd,color=next(colors22))
            elif (lons2[i-itera]>150 and lons2[i])<-150:
                ax2.plot([-180,lons2[i]],[alts2[i-itera],alts2[i]],linewidth=lnwd,color=next(colors22))
            else:
                ax2.plot([lons2[i-itera],lons2[i]],[alts2[i-itera],alts2[i]],linewidth=lnwd,color=next(colors22))
    for i in range(itera,idx3+1,itera):
        if  track_model3[i]['date'].month==9 and 3<track_model3[i]['date'].day<16:
            if (lons3[i-itera]-lons3[i]<-100): 
                ax2.plot([180,lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color='red',marker='x')
            elif (lons3[i-itera]>150 and lons3[i])<-150:
                ax2.plot([-180,lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color='red',marker='x')
            else:
                ax2.plot([lons3[i-itera],lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color='red',marker='x')
        elif i==int(idx3/itera)*itera:
            if (lons3[i-itera]-lons3[i]<-100): 
                ax2.plot([180,lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color=next(colors32),label='Bubble B2')
            elif (lons3[i-itera]>150 and lons3[i])<-150:
                ax2.plot([-180,lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color=next(colors32),label='Bubble B2')
            else:
                ax2.plot([lons3[i-itera],lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color=next(colors32),label='Bubble B2')
        else:
            if (lons3[i-itera]-lons3[i]<-100): 
                ax2.plot([180,lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color=next(colors32))
            elif (lons3[i-itera]>150 and lons3[i])<-150:
                ax2.plot([-180,lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color=next(colors32))
            else:
                ax2.plot([lons3[i-itera],lons3[i]],[alts3[i-itera],alts3[i]],linewidth=lnwd,color=next(colors32))
...
##
    ...
    lev=ialts
    cardinal_level=True
    txt='Trajectories of three followed vortex on an zonal ozone map (10**-6 g/kg) at '+str(round(altitude,2))+' km'
    log=False
    clim=(None,None)
    figsize=(11,4)
    cmap=mymap
    ...
    for i in range(itera,idx1,itera):
        if  track_model1[i]['date'].month==9 and 3<track_model1[i]['date'].day<16:
            if (lons1[i-itera]-lons1[i]<-100): 
                ax.plot([180,lons1[i]],[lats1[i-itera],lats1[i]],linewidth=lnwd,color='black',marker='x')
            elif (lons1[i-itera]>150 and lons1[i])<-150:
                ax.plot([-180,lons1[i]],[lats1[i-itera],lats1[i]],linewidth=lnwd,color='black',marker='x')
            else:
                ax.plot([lons1[i-itera],lons1[i]],[lats1[i-itera],lats1[i]],linewidth=lnwd,color='black',marker='x')
        else:
            if (lons1[i-itera]-lons1[i]<-100): 
                ax.plot([180,lons1[i]],[lats1[i-itera],lats1[i]],linewidth=lnwd,color=next(colors1))
            elif (lons1[i-itera]>150 and lons1[i])<-150:
                ax.plot([-180,lons1[i]],[lats1[i-itera],lats1[i]],linewidth=lnwd,color=next(colors1))
            else:
                ax.plot([lons1[i-itera],lons1[i]],[lats1[i-itera],lats1[i]],linewidth=lnwd,color=next(colors1))
    for i in range(itera,idx2,itera):
        if  track_model2[i]['date'].month==9 and 3<track_model2[i]['date'].day<16:
            if (lons2[i-itera]-lons2[i]<-100): 
                ax.plot([180,lons2[i]],[lats2[i-itera],lats2[i]],linewidth=lnwd,color='blue',marker='x')
            elif (lons2[i-itera]>150 and lons2[i])<-150:
                ax.plot([-180,lons2[i]],[lats2[i-itera],lats2[i]],linewidth=lnwd,color='blue',marker='x')
            else:
                ax.plot([lons2[i-itera],lons2[i]],[lats2[i-itera],lats2[i]],linewidth=lnwd,color='blue',marker='x')
        else:
            if (lons2[i-itera]-lons2[i]<-100): 
                ax.plot([180,lons2[i]],[lats2[i-itera],lats2[i]],linewidth=lnwd,color=next(colors2))
            elif (lons2[i-itera]>150 and lons2[i])<-150:
                ax.plot([-180,lons2[i]],[lats2[i-itera],lats2[i]],linewidth=lnwd,color=next(colors2))
            else:
                ax.plot([lons2[i-itera],lons2[i]],[lats2[i-itera],lats2[i]],linewidth=lnwd,color=next(colors2))
    for i in range(itera,idx3,itera):
        if  track_model3[i]['date'].month==9 and 3<track_model3[i]['date'].day<16:
            if (lons3[i-itera]-lons3[i]<-100): 
                ax.plot([180,lons3[i]],[lats3[i-itera],lats3[i]],linewidth=lnwd,color='red',marker='x')
            elif (lons3[i-itera]>150 and lons3[i])<-150:
                ax.plot([-180,lons3[i]],[lats3[i-itera],lats3[i]],linewidth=lnwd,color='red',marker='x')
            else:
                ax.plot([lons3[i-itera],lons3[i]],[lats3[i-itera],lats3[i]],linewidth=lnwd,color='red',marker='x')
        else:
            if (lons3[i-itera]-lons3[i]<-100): 
                ax.plot([180,lons3[i]],[lats3[i-itera],lats3[i]],linewidth=lnwd,color=next(colors3))
            elif (lons3[i-itera]>150 and lons3[i])<-150:
                ax.plot([-180,lons3[i]],[lats3[i-itera],lats3[i]],linewidth=lnwd,color=next(colors3))
            else:
                ax.plot([lons3[i-itera],lons3[i]],[lats3[i-itera],lats3[i]],linewidth=lnwd,color=next(colors3))
    plt.savefig(savfile+str(variable)+'_alt.png',dpi=300,bbox_inches='tight',format='png')
    plt.show()



# nametrackmodel1 = '/home/hlestrel/stageM1-vortex/track_model_'+initials+'debutest'
# nametrackmodel2 = '/home/hlestrel/stageM1-vortex/track_model_'+initials+'_BulleB1_ref_tot'
# with gzip.open(nametrackmodel1,'rb') as f:
#     track_model1 = pickle.load(f)
# with gzip.open(nametrackmodel2,'rb') as f:
...
#     
# track_modelY={}
# track_modelY['dates']=[track_model1[132]['date']]
# track_modelY['lons']=[track_model1[132]['lon']]
# track_modelY['lats']=[track_model1[132]['lat']]
# track_modelY['alts']=[track_model1[132]['alt']]
# for i in range(135,174,3):
...
# for i in range(175,600):
#     track_modelY['dates'].append(track_model2[i]['date'])
#     track_modelY['lons'].append(track_model2[i]['lon'])
#     track_modelY['lats'].append(track_model2[i]['lat'])
#     track_modelY['alts'].append(track_model2[i]['alt'])
# pickle.dump(track_modelY,open('/home/hlestrel/stageM1-vortex/Vortex-track_CAN2017_B1.pkl','wb'))